...
ZEND_API zend_class_entry *do_bind_inherited_class(const zend_op_array *op_array, const zend_op *opline, HashTable *class_table, zend_class_entry *parent_ce, zend_bool compile_time TSRMLS_DC) /* {{{ */
{
	...
	if (!compile_time) {
		zend_verify_class_return_type_variance(ce, parent_ce TSRMLS_CC);
	}
...
static zend_bool class_uses_return_types(const zend_op_array *op_array, const zend_op *opline, const HashTable *class_table)
{
	const zval *op1 = &CONSTANT_EX(op_array, opline->op1.constant);
	const zend_class_entry *ce = zend_hash_find_ptr(class_table, Z_STR_P(op1));
	const zend_function *func;

	assert(ce != NULL);

	ZEND_HASH_FOREACH_PTR(&ce->function_table, func) {
		if (func->common.return_type.kind != IS_UNDEF) {
			return 1;
		}
		...
		case ZEND_DECLARE_INHERITED_CLASS:
			{
				...
				if (class_uses_return_types(CG(active_op_array), opline, CG(class_table))) {
					return;
				}
...
void zend_compile_return(zend_ast *ast TSRMLS_DC) /* {{{ */
{
	...
	if (CG(active_op_array)->return_type.kind != IS_UNDEF) {
...
void zend_compile_class_decl(zend_ast *ast TSRMLS_DC) /* {{{ */
{
	...
	if (ce->constructor) {
		...
		if (ce->constructor->common.return_type.kind != IS_UNDEF) {
			zend_error_noreturn(E_COMPILE_ERROR,
	...
	if (ce->destructor) {
		...
		} else if (ce->destructor->common.return_type.kind != IS_UNDEF) {
			zend_error_noreturn(E_COMPILE_ERROR,
	...
	if (ce->clone) {
		...
		} else if (ce->clone->common.return_type.kind != IS_UNDEF) {
			zend_error_noreturn(E_COMPILE_ERROR,
